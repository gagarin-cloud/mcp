...
export function registerTools(server: McpServer, api: Api): void {
  ...
  // ─── getting access ──────────────────────────────────────────────────────
  //
  // Both unauthenticated, and both here rather than left to the CLI, because an
  // agent that has just been pointed at this server has no credential yet and
  // "install a Go binary first" is a wall at the wrong moment.

  server.registerTool(
    'login',
    {
      title: 'Ask a human for access',
      description:
        'Asks a human to authorise this agent, and returns a short code. Creates no account: a ' +
        'human clicking a button in an email is what creates it. Ask your human for their ' +
        'address — never guess it. Then relay BOTH halves of the answer to them, because gagarin ' +
        'will not say which applies and neither should you: if the address already has an ' +
        'account, a link is in their inbox now; if it does not, nothing has been emailed, and ' +
        'they must send a mail to the signup address in the answer, with the code in the ' +
        'subject, to be sent one. gagarin does not write to an address until that address writes ' +
        'to it. The code should match the one in whichever email they get. Then poll `claim`. It ' +
        'is the same call either way — there is no separate signup.',
      inputSchema: {
        email: z.string().describe("your human's email address, as they gave it to you"),
        client: z
          .string()
          .optional()
          .describe(
            'what to name this requester in the email a human is about to read, e.g. ' +
              '"claude code on marina\'s laptop". A blank one makes their decision harder.',
          ),
      },
    ...
    ({ email, client }) =>
      attempt(() =>
        api.call('/v1/signup', {
          method: 'POST',
          body: { email, client: client ?? "an agent, through gagarin's MCP server" },
          authenticated: false,
        }),
      ),
  );
  ...
  server.registerTool(
    'claim',
    {
      title: 'Collect the credential once approved',
      description:
        'Exchanges the code from `login` for a credential, once a human has clicked. Answers ' +
        'status "pending" with a retry_after while nobody has, so poll it rather than treating ' +
        'that as a failure; the request expires in ten minutes. The credential comes back exactly ' +
        'once and is never readable again. Put it in this server\'s Authorization header as ' +
        '`Bearer <credential>` (remote) or in GAGARIN_TOKEN (stdio) — or hand it to your human to ' +
        'paste into their MCP configuration, which keeps it out of a transcript.',
      inputSchema: {
        code: z.string().describe('the code `login` returned, e.g. ABCD-1234'),
        client: z.string().optional().describe('what to call this machine in the credential list'),
      },
      annotations: { readOnlyHint: false, idempotentHint: false, openWorldHint: true },
    },
    ({ code, client }) =>
